...
- **Синхронізація з Numista** — заповнює вагу/діаметр/товщину/гурт/тираж по всій базі; живий лог із деталями кожного запиту, лічильник і попередження про місячну квоту API (2000 запитів)
...
- **Сортування** — за датою, країною, роком, або комбіновано (країна+рік+номінал, країна+номінал+рік)
...
- **Прапори та ISO-коди** — 100+ сучасних країн через ISO-коди; 18 зниклих держав (СРСР, Третій Рейх, Австро-Угорщина, Родезія, Малайя, Цейлон, ОАР та інші) показують власний історичний SVG-прапор, а не прапор країни-наступниці
...
| `GET` | `/api/numista-sync` | Синхронізація ваги/діаметру/тиражу тощо через Numista (SSE-потік) |
| `GET` | `/api/numista-quota` | Використання місячної квоти Numista API |
...
│   │   ├── CountryFlag.tsx         # Прапори країн (історичні SVG → ISO-код → emoji fallback)
│   │   └── ServicePage.tsx         # Налаштування AI, PDF-експорт, REST-документація
│   └── utils/
│       ├── countryUtils.ts         # Маппінг країн → ISO-коди / прапори (100+ країн)
│       ├── historicalFlags.ts      # Прапори зниклих держав (СРСР, Родезія, Малайя тощо)
